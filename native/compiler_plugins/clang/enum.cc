...
namespace {
    ...
    public:
        ...
        virtual void run(const MatchFinder::MatchResult &Result)
        {
            ...
            emit_conversion_warning(diagEngine, expr->getSourceRange().getBegin(), enum_decl ? enum_decl->getNameAsString() : "<unknown>");
        }
    ...
    public:
        ...
        void check(ASTContext *context, const EnumType *enumType, const SwitchStmt *stmt)
        {
                 ...
                 switchCase = switchCase->getNextSwitchCase())
            {
                ...
                if (!caseExpr) {
                    emit_case_warning(diagEngine, caseStmt->getSourceRange().getBegin(),
...
static FrontendPluginRegistry::Add<CheckEnumConversionAction> registerEnum("enum", "check enum conversion");
