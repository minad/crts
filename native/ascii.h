 ...
 * The ctype.h functions should be considered legacy since they
 * only work with legacy single byte encodings.
...
CHI_INL bool chiHexDigit(int c)  { return chiDigit(c) || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f'); }
