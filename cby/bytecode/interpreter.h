...
OP_BEGIN(choice)
    ...
    const Chili a = REG[FETCH8];
    const Chili b = REG[FETCH8];
...
OP_BEGIN(thk0)
    ...
    NEW(thunk, CHI_THUNK, CHI_SIZEOF_WORDS(CbyFn) + 0);
    *res = thunk; // Note: *res is written before fields are read, in order to support recursion
    CbyFn* fn = NEW_PAYLOAD(CbyFn, thunk);
    fn->cont = chiFromCont(&interpThunk);
    fn->val = thunk;
...
OP_BEGIN(thk1)
    ...
    NEW(thunk, CHI_THUNK, CHI_SIZEOF_WORDS(CbyFn) + 1);
    *res = thunk; // Note: *res is written before fields are read, in order to support recursion
    CbyFn* fn = NEW_PAYLOAD(CbyFn, thunk);
    fn->cont = chiFromCont(&interpThunk);
    fn->val = thunk;
...
OP_BEGIN(thk2)
    ...
    NEW(thunk, CHI_THUNK, CHI_SIZEOF_WORDS(CbyFn) + 2);
    *res = thunk; // Note: *res is written before fields are read, in order to support recursion
    CbyFn* fn = NEW_PAYLOAD(CbyFn, thunk);
    fn->cont = chiFromCont(&interpThunk);
    fn->val = thunk;
...
OP_BEGIN(thk3)
    ...
    NEW(thunk, CHI_THUNK, CHI_SIZEOF_WORDS(CbyFn) + 3);
    *res = thunk; // Note: *res is written before fields are read, in order to support recursion
    CbyFn* fn = NEW_PAYLOAD(CbyFn, thunk);
    fn->cont = chiFromCont(&interpThunk);
    fn->val = thunk;
...
OP_BEGIN(thk4)
    ...
    NEW(thunk, CHI_THUNK, CHI_SIZEOF_WORDS(CbyFn) + 4);
    *res = thunk; // Note: *res is written before fields are read, in order to support recursion
    CbyFn* fn = NEW_PAYLOAD(CbyFn, thunk);
    fn->cont = chiFromCont(&interpThunk);
    fn->val = thunk;
...
OP_BEGIN(ret2)
    const Chili v0 = REG[FETCH8];
    const Chili v1 = REG[FETCH8];
    _Static_assert(2 <= CBY_CONTEXT_SIZE, "Too many return values");
    INSN_LEAVE;
    if (chiToCont(SP[-1]) != &interpCont) {
        ASET(0, v0);
        ASET(1, v1);
...
OP_BEGIN(ret3)
    const Chili v0 = REG[FETCH8];
    const Chili v1 = REG[FETCH8];
    const Chili v2 = REG[FETCH8];
    _Static_assert(3 <= CBY_CONTEXT_SIZE, "Too many return values");
    INSN_LEAVE;
    if (chiToCont(SP[-1]) != &interpCont) {
        ASET(0, v0);
        ASET(1, v1);
...
OP_BEGIN(ret4)
    ...
    const Chili v1 = REG[FETCH8];
    const Chili v2 = REG[FETCH8];
    const Chili v3 = REG[FETCH8];
    _Static_assert(4 <= CBY_CONTEXT_SIZE, "Too many return values");
    INSN_LEAVE;
    if (chiToCont(SP[-1]) != &interpCont) {
        ASET(0, v0);
        ASET(1, v1);
...
OP_END
OP_BEGIN(int16ToUInt16)
    Chili* const res = REG + FETCH8;
    const Chili a0 = REG[FETCH8];
    *res = chiFromUInt16(chi_Prim_int16ToUInt16(chiToInt16(a0)));
OP_END
OP_BEGIN(int32Add)
    Chili* const res = REG + FETCH8;
    const Chili a0 = REG[FETCH8];
...
OP_END
OP_BEGIN(int8ToUInt8)
    Chili* const res = REG + FETCH8;
    const Chili a0 = REG[FETCH8];
    *res = chiFromUInt8(chi_Prim_int8ToUInt8(chiToInt8(a0)));
OP_END
OP_BEGIN(intAdd)
    Chili* const res = REG + FETCH8;
    const Chili a0 = REG[FETCH8];
...
OP_END
OP_BEGIN(uint16ToInt16)
    Chili* const res = REG + FETCH8;
    const Chili a0 = REG[FETCH8];
    *res = chiFromInt16(chi_Prim_uint16ToInt16(chiToUInt16(a0)));
OP_END
OP_BEGIN(uint32Add)
    Chili* const res = REG + FETCH8;
    const Chili a0 = REG[FETCH8];
...
OP_END
OP_BEGIN(uint8ToInt8)
    Chili* const res = REG + FETCH8;
    const Chili a0 = REG[FETCH8];
    *res = chiFromInt8(chi_Prim_uint8ToInt8(chiToUInt8(a0)));
OP_END
OP_BEGIN(lazyForce)
    const Chili thk = REG[FETCH8];
