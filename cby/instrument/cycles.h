...
typedef struct {
    uint64_t begin, insn[OPCODE_COUNT];
} cyclesData;
...
CHI_INL void cyclesLeave(ChiProcessor* CHI_UNUSED(proc), cyclesData* CHI_UNUSED(local), Chili CHI_UNUSED(fn), Chili* CHI_UNUSED(frame)) {}
CHI_INL void cyclesProcStart(ChiProcessor* CHI_UNUSED(proc), cyclesData* CHI_UNUSED(local), cyclesData* CHI_UNUSED(global)) {}
CHI_INL void cyclesSetup(CbyInterpreter* CHI_UNUSED(interp), cyclesData* CHI_UNUSED(global)) {}
